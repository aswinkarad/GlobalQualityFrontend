        ...
        <Transition name="slide-fade">
            <div class="mt-4 mb-2" v-if="filterDisplay">
                <v-row>
                    ...
                    <v-col cols="12" md="2">
                        <v-text-field v-model.number="minMessageCount" clearable hide-details variant="outlined"
                            density="compact" label="Min Messages" type="number" min="0"></v-text-field>
                    </v-col>
        ...
        <v-table class="rounded-lg mt-4">
            <thead>
                <tr>
                    ...
                    <th class="text-left" style="width:5%">Messages</th>
                    <th class="text-left" v-if="showMessage">Comments</th>
            ...
            <tbody>
                <tr v-for="(item) in filteredServiceList" :key="item.id">
                    ...
                    <td class="message" @click="openMessageDialog(item)">
                        <span class="circle-badge">{{ item.messageCount || 0 }}</span>
                    </td>
                    ...
                    <td>
                        <v-menu transition="scale-transition">
                            ...
                            <v-list dense>
                                ...
                                <v-list-item @click="openMessageListForService(item)" v-if="showToolbar">
                                    <v-list-item-title>
...
export default {
 ...
 currentUser() {
       ...
       filteredServiceList() {
        ...
        // If user is a Technician, ONLY show their assigned services (no other filters)
        if (this.currentUser && this.currentUser.role && this.currentUser.role[0] === 'Technician') {
            return filteredList.filter(item => item.userId === this.currentUser.id);
        }
        
        // For non-Technicians, apply all the existing filters
        // Filter by Priority
        ...
        if (this.minMessageCount !== null && this.minMessageCount >= 0) {
            filteredList = filteredList.filter(item =>
                (item.messageCount || 0) >= this.minMessageCount
            );
        }
    ...
    showFilterSection() {
        return this.currentUser?.role?.[0] !== 'Technician';
    }
    ...
    watch: {
        // Watch for changes in MessageList and log all messages
        MessageList: {
            handler(newMessages, oldMessages) {
                if (newMessages && newMessages.length > 0) {
                    ...
                    // Group messages by service request
                    const messagesByService = newMessages.reduce((acc, msg) => {
                        const serviceId = msg.serviceRequestId;
                        if (!acc[serviceId]) {
                            acc[serviceId] = [];
                        }
                        acc[serviceId].push(msg);
                        return acc;
                    }, {});
                    Object.keys(messagesByService).forEach(serviceId => {
    ...
    methods: {
        ...
        ...mapActions('message', ['ADD_CEOMESSAGE', 'GET_MESSAGE_LIST']),

        // Enhanced message logging methods
        async logAllMessages() {
            try {
        ...
        openMessageDialog(item) {
            // Filter messages for this specific service
            const serviceMessages = this.MessageList.filter(msg =>
                msg.serviceRequestId === item.id
            );
            // console.log(`Messages for Service ${item.id}:`, serviceMessages);
        },

        openMessageListForService(item) {
            // Log messages for this service before navigation
            const serviceMessages = this.MessageList.filter(msg =>
                msg.serviceRequestId === item.id
            );
        ...
        getApprovalColor(status) {
            ...
            return approvalColors[status] || '#ff9800';
        },
        ...
        async openAddService() {
            ...
            try {
                ...
                this.mssg = error.message || 'Error loading data for adding service request';
                this.snackColor = 'error';
                this.snackbar = true;
            }
        },
        ...
        async addService(payload) {
            try {
                ...
                this.dialog = false;
                await this.fetchServiceList();
                // Refresh messages after adding service
                await this.logAllMessages();
            } catch (error) {
        ...
        async openEditDialog(item) {
            try {
                await this.GET_SERVICE_WITH_ID(item.id);
                ...
                this.mssg = error.message || 'Error loading data for editing service request';
                this.snackColor = 'error';
                this.snackbar = true;
            }
        },
        ...
        async updateService(payload) {
            try {
                ...
                this.snackColor = 'success';
                this.snackbar = true;
                // Refresh messages after updating service
                await this.logAllMessages();
            } catch (error) {
        ...
        async deleteService() {
            try {
                ...
                await this.fetchServiceList();
                // Refresh messages after deleting service
                await this.logAllMessages();
            } catch (error) {
        ...
        //     try {
        //         const response = await this.GET_SERVICE_LIST({ size: 500 });
        //         this.allServiceList = [...this.serviceList];
        //         // Also fetch messages to calculate counts
        //         await this.logAllMessages();

        //         // If messageCount is not provided by backend, calculate it
        //         if (this.MessageList && this.MessageList.length > 0) {
        //             this.allServiceList = this.allServiceList.map(service => {
        //                 const count = this.MessageList.filter(msg =>
        //                     msg.serviceRequestId === service.id
        //                 ).length;
        //                 // console.log(`Service ${service.id} has ${count} messages`);
        //                 return { ...service, messageCount: count };
        //             });
        //         }
        //     } catch (error) {
        //         // console.error('Error fetching service list:', error);
        //         this.mssg = error.message || 'Failed to load service requests';
        //         this.snackColor = 'error';
        //         this.snackbar = true;
        //     }
        // },
        ...
        try {
            ...
            // Debug logging
            if (this.currentUser?.role?.[0] === 'Technician') {
                const userServices = this.allServiceList.filter(item => 
                    item.userId === this.currentUser.id
                );
                console.log(`Technician ${this.currentUser.username} (ID: ${this.currentUser.id}) - Assigned Services:`, userServices.length);
                console.log('Assigned Services Details:', userServices);
            }
            
            // Also fetch messages to calculate counts
            await this.logAllMessages();

            // Calculate message counts if not provided by backend
            if (this.MessageList && this.MessageList.length > 0) {
                this.allServiceList = this.allServiceList.map(service => {
                    const count = this.MessageList.filter(msg =>
                        msg.serviceRequestId === service.id
        ...
        // },
         searchSeriveRe(val) {
        if (this.currentUser?.role?.[0] === 'Technician') {
            // For Technicians, search is disabled or works only on their assigned services
            // You can choose to disable search entirely or allow it
            this.serviceSearchVal = '';
            return;
        }
        this.serviceSearchVal = val || '';
    },
        ...
        async sendApprovalRequest() {
            if (!this.message || !this.selectedServiceItem) {
                // console.error('Missing required fields for approval request');
            ...
            try {
                ...
                await this.fetchServiceList(); // Re-fetch to update message counts
                await this.logAllMessages(); // Log updated messages
            } catch (error) {
    ...
    async mounted() {
        try {
            ...
            this.mssg = 'Error loading initial data.';
            this.snackColor = 'error';
            this.snackbar = true;
        }
    }
...
.v-table>.v-table__wrapper>table>thead>tr>th:last-child {
    ...
    /* Fixed width to match button size */
...
.v-table>.v-table__wrapper>table>tbody>tr>td:last-child .v-btn {
    ...
    /* Match Vuetify icon button size */
    height: 32px !important;
    /* Consistent height */
    min-width: unset !important;
    /* Override Vuetify's min-width */
}
